"""
mini WSGI Server
version: 0.11 (with wsgi_app_frame_v011.py)
        no change from v0.10
functionality:
...
DONE:
    ...
    6. add configure file:
        + realize find /static and /dynamic path
    7. pseudo-static
        + change ***.py as judgement standard to seperate
        dynamic and static to ***.html
    8. add data
...
class WSGI_SERVER(object):
    ...
    def request_handler(self, servant):
        ...
        if not request_src_path.endswith('.html'):
            try:
