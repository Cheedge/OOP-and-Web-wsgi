"""
mini WSGI Server
version: 0.5 (with wsgi_app_frame_v05.py)
functionality:
...
class WSGI_SERVER(object):
    ...
    def request_handler(self, servant):
        ...
        if not request_src_path.endswith('.py'):
            try:
                f = open('./static'+request_src_path, 'rb')
...
def main():
    ...
    TODO:
        get params from command:
            python port module(frame):app
        use sys.arg to get params from run command
    """
    # PORT = 8080
    if len(sys.argv)==3:
        try:
    ...
    mini_wsgi_server = WSGI_SERVER(PORT, app)
