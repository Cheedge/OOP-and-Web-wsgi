"""
mini WSGI Server
version: 0.1 (with wsgi_app_frame_v01.py)
functionality:
...
class WSGI_SERVER(object):
    ...
    def request_handler(self, servant):
        ...
        if not request_src_path.endswith('.py'):
            ...
            request_line = "HTTP/1.1 200 OK\r\n"
            request_head = ""
            # request_body = wsgi_app_frame.login()
            request_body = wsgi_app_frame_v01.application(request_src_path)
            response = request_line + request_head + '\r\n' + request_body
            servant.send(response.encode('utf-8'))

        servant.close()

    def set_response_head(self, head, status):
        """
        get status from App_Frame, construct head
        then send to App_Frame
        """
        pass
